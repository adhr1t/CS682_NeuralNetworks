...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ## 1
        # embed caption
        embed_caption = self.embedding(captions) # N, T –> N, T, W
        # These dimensions are captions dimensions and wordvec_dim

        # add positional encoding to caption
        # maintains N, T, W dimensions
        embed_caption_encode = self.positional_encoding(embed_caption)

        # project image features into same dimension
        # having an issue here. N, D –> N, W –>
        # nn.Linear powering self.visual_projection is (N, D) * (D, W) = (N, W)
        # Need to transform (N, W) to N, T, E
        features_project = self.visual_projection(features).reshape(N, 1, -1)


        ## 2
        # prepare a mask tgt_mask for timesteps in captions
        # tgt_mask: the parts of the target sequence to mask, of shape (T, T)
        mask_ones = torch.ones(T, T)
        target_mask = torch.tril(input = mask_ones)


        ## 3
        # apply decoder features on the text and the image embeddings
        # having an issue with dimensions being fed into memory. Expecting N, T, E; getting N, T
        decoded_text_images = self.transformer(tgt = embed_caption_encode, memory = features_project, tgt_mask = target_mask)

        # Project the output to scores per token
        scores = self.output(decoded_text_images)
